...
prog def masala_merge
{
  ...
  qui {
    ...
    if _rc {
      di "WARNING: masala_merge: There were no matches. Empty output file will be saved."
      clear
...
prog def fix_spelling
{
  ...
  if !mi("`replace'") {
    ...
    di "Spelling fixes and Masala-Levenshtein distances:"
...
prog def masala_review
{
  ...
  if `r(N)' > 1 {
    di "These are high cost matches, with no good alternatives. keep_master is 1."
  ...
  if `r(N)' {
    di "This is a sorted list of some places that did not match. Review for ideas on how to improve"
...
prog def masala_process
{
  ...
  /* show merge result */
  di "Results of masala_merge (counting unique strings only): "
